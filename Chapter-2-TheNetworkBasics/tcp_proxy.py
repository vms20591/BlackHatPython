...
def proxy_handler(client_socket, remote_host, remote_port, receive_first):
  ...
  if receive_first:
    ...
    # send if we have data to send our local client
...
def receive_from(connection):
  ...
  start = time.time()
  # setting timeout of 2 seconds and may need to be adjusted
  connection.settimeout(2)

  try:
    # keep reading into buffer until no more data
    while True:
...
# modify response destined for localhost
...
def main():
  ...
  remoreport = int(sys.argv[4])
  
  # this tells our proxy to connect and receive data
  receive_first = True if "True" in sys.argv[5] else False

  # spin listening socket
  server_loop(localhost, localport, remotehost, remoreport, receive_first)
